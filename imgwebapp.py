...
# We dont want to load the model for every inference. Thats why we are caching this. 
# i.e. load the model and keep it in the cache. Because loading the model taks time to load again and again.
...
if path is not None:
    # Get the bytes of the image(Byte array object for tf)
    content = requests.get(path).content

    st.write("Predicted Class :")
    
    # Spinner object - waits till the classification happens.
